    ...
    for(iter in 2:(tot_iter+1)){
      ...
      for(j in 1:J){ #a
        for(x in which(unique_tab[j,]>0)){ #b
          ...
          temp_temp_0 = unique(X[(r[iter,,]==0)])
          tempX0 = temp_temp_0[!is.na(temp_temp_0)]
      ...
      if(n_temp>0){ #rest empty?
        ...
        for (iter_MH in 1:niter_MH){
          ...
          if(epsilon_MH<alpha_prop && alpha_prop<Max_val){
            ...
            Acc_prob_alpha = a_alpha0*(log_alpha_prop - log_alpha_old)+
              b_alpha0*(alpha_old - alpha_prop)
          ...
          if(epsilon_MH<sigma_prop && sigma_prop<1-epsilon_MH){
            ...
            # Prior and Jacobian part
            Acc_prob_sigma = a_sigma0*(log_sigma_prop - log_sigma_old)+
              b_sigma0*(log(1-sigma_prop) - log(1-sigma_old))
        ...
        alpha[1] = rgamma(1, shape = a_alpha0, rate = b_alpha0)
        sigma[1] = rbeta(1, a_sigma0, b_sigma0)
