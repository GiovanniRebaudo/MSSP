...
generate_zipf_reorder<- function(param = c(rep(1.3, 2), rep(2, 6)), 
                                 tot_species = 3000, j_species = 2500, 
                                 common_most_common = 30, seed = 0){
  ## The function returns the pmfs of a number J of zipf distributions
  ## J is implicitly defined by the length of the input param
  ##inputs: 
  ## param[j] = zipf parameter of pop j 
  ...
  for(j in 1:length(param)){
    ...
    truth[[j]][species] = (1 / c(1:j_species))**(param[j])
    truth[[j]][1:(common_most_common)] = 
      sample(truth[[j]][1:(common_most_common)], 
             (common_most_common))
    truth[[j]][(common_most_common+1):tot_species] = 
                    sample(truth[[j]][(common_most_common+1):tot_species], 
                            (tot_species - common_most_common))
  }
